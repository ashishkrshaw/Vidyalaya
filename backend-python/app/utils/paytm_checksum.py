"""
Paytm Checksum Library
Source: Official Paytm Python Sample
...
def generate_refund_checksum(param_dict, merchant_key, salt=None):
    """Generate checksum for refund requests"""
    for i in param_dict:    
        if("|" in param_dict[i]):
            param_dict = {}
            exit()
...
def generate_checksum_by_str(param_str, merchant_key, salt=None):
    """Generate checksum from string (for JSON body)"""
...
def verify_checksum(param_dict, merchant_key, checksum):
    ...
    # Remove checksum from dict before verification
    if 'CHECKSUMHASH' in param_dict:
        param_dict.pop('CHECKSUMHASH')

    # Get salt from existing checksum
...
def __get_param_string__(params):
    """Convert params dict to pipe-separated string"""
    params_string = []
